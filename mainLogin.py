...
bigishBoldFont = pygame.font.SysFont("comicsansms", 80, True)
biggerBoldFont = pygame.font.SysFont("comicsansms", 70, True)
...
# with open("levels2.txt", "r") as fileName:
#     for value in fileName.read().split("\n"):
#         if username == value.split(": ")[0]:
#             farthestLevel = int(value.split(": ")[1])
#             found = True
#
#     if not found:
#         with open("levels2.txt", "a") as file:
...
def login():
    ...
    # global farthestLevel, level, username
    #
    # found = False
    ...
    #             if username in file:
    #                 farthestLevel = int(file[file.index(username) + 1])
    #                 found = True
    #
    #             if not found:
    #                 with open("levels2.txt", "a") as file:
...
def nextLevel():
    ...
    if level > farthestLevel:
        with open('levels2.txt', 'r') as fileName2:
            filedata = fileName2.read().split("\n")
            print(filedata)

        # Replace the target string
        print(filedata.index(username) + 1)
        filedata[filedata.index(username) + 1] = level
        print(filedata)

        # Write the file out again
        with open('levels2.txt', 'w') as fileName2:
            fileName2.write(str(filedata)[1:-1].replace("'", "").replace(", ", "\n"))
...
async def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if gameState not in {GameMode.MENU, GameMode.LOGIN}:
                if event.type == pygame.KEYDOWN:
        ...
        if gameState == GameMode.LOGIN:
            ...
            loginText = bigishBoldFont.render("Enter Username: ", True, color["Red"])
            usernameText = biggerBoldFont.render(username, True, color["Light_Grey"])
            display_surface.blit(loginText, loginText.get_rect(center=(420, 150)))
            display_surface.blit(usernameText, usernameText.get_rect(center=(400, 275)))
